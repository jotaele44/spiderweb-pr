...
async def spatial_capabilities():
    ...
    cesium is True as of Phase 2: workbench/priis-v1/app ships a lazily-loaded
    CesiumRegionalRuntime behind the SpatialRuntime seam. It is a regional
    *shell* only — ellipsoid globe with no imagery and no terrain (terrain is
    Phase 4, gated on the GEBCO vertical-datum work), and it falls back to
    MapLibre when WebGL is unavailable. Note this flag describes the frontend
    build, not this backend process.
    """
    return {
        "runtimes": {"maplibre": True, "cesium": True},
...
@app.get("/spatial/boundaries/{boundary_id}.geojson")
async def spatial_boundary_geometry(boundary_id: str):
    """Actual geometry for a resolved/provisional boundary registry entry.

    Looks up boundary_id in configs/spatial_boundaries.yaml, follows its
    geometry_ref (a data/ file, regenerated by
    server/ingestion/ingest_pr_boundaries.py — gitignored, not committed) and
    ...
    boundaries = _BOUNDARY_REGISTRY.get("boundaries") or []
    ...
    if not path.exists():
        raise HTTPException(
            ...
            f"run server/ingestion/ingest_pr_boundaries.py",
        )
    return FileResponse(str(path), media_type="application/geo+json")
