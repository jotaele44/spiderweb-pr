 ...
 *
 * Reads from window.flightData (JSON export produced by run_all.py --export-json).
 * Embedded via dashboard.html using Babel standalone + React CDN — no build step.
 *
 * Tabs:
 *   Overview          — summary stats, top operators, recent alerts
 *   Aircraft Catalog  — per-aircraft cards with mission and confidence
 *   Flight Log        — sortable flight history table
 *   Intelligence      — full alert feed with severity coloring
 */

const { useState, useMemo } = React;

// ── Icons (lucide-react subset via CDN) ─────────────────────────────────────
...
const SeverityBadge = ({ severity }) => (
  <span className={`px-2 py-0.5 rounded text-xs font-semibold ${severityStyle[severity] || severityStyle.INFO}`}>
    {severity}
  </span>
);
...
const SortTh = ({ label, field, sort, onSort }) => (
  <th
    className="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase cursor-pointer select-none hover:text-gray-700"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Aircraft tracked"    value={aircraft_profiles.length || new Set(flights.map(f => f.callsign)).size} icon="plane" />
              ...
              <div key={op}>
                ...
                <div className="h-2 bg-gray-100 rounded">
                  <div
                    ...
                    style={{ width: `${(cnt / maxOpCount) * 100}%` }}
                  />
                </div>
              </div>
            ))}
  ...
  return (
    <div className="space-y-4">
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search callsign, operator, mission…"
          value={query}
      ...
      {profiles.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {profiles.map((p, i) => (
            <div key={i} className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
  ...
  return (
    <div className="space-y-3">
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Filter by callsign, mission, airport…"
          value={query}
          onChange={e => setQuery(e.target.value)}
          className="w-full pl-8 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-300"
        />
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow">
        ...
        {sorted.length > 200 && (
          <p className="text-xs text-gray-400 text-center py-2">Showing first 200 of {sorted.length} results</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((a, i) => (
            <div key={i} className={`bg-white rounded-lg shadow p-3 border-l-4 ${
              a.severity === "CRITICAL" ? "border-red-700" :
              a.severity === "HIGH"     ? "border-red-400" :
              a.severity === "MEDIUM"   ? "border-yellow-400" :
              a.severity === "LOW"      ? "border-blue-400" : "border-gray-300"
            }`}>
...
const App = ({ data = window.flightData || {} }) => {
  ...
  const tabs = [
    ...
    { id: "catalog",   label: "Aircraft Catalog" },
    { id: "flightlog", label: "Flight Log" },
    { id: "intel",     label: "Intelligence Analysis" },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        ...
        {tab === "catalog"   && <AircraftCatalogTab data={data} />}
        {tab === "flightlog" && <FlightLogTab       data={data} />}
        {tab === "intel"     && <IntelligenceTab    data={data} />}
      </main>
    </div>
  );
};

// Mount when loaded via dashboard.html
