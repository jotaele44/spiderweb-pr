...
def main() -> int:
    ...
    if manifest.get("producer_repo") != "spiderweb-pr":
        problems.append("producer_repo mismatch")
    if (
        manifest.get("cross_repo", {}).get("identity_default")
    ...
    if manifest.get("storage", {}).get("ownership") != "REPO_LOCAL":
    ...
    print(
        json.dumps(
            {
                "ok": True,
                "producer_repo": "spiderweb-pr",
                "contract": "federation-spatial-contract/1.0",
            },
            indent=2,
        )
    )
