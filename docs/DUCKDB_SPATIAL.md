...
Keep the filename `spatial.duckdb_extension`: DuckDB derives the native
entrypoint name from it. A renamed copy can fail to load despite identical
bytes. An explicitly configured extension that fails to load is a test failure,
not an optional-coverage skip.
...
Municipio lookup returns the raw source `NAME` only for one containing row.
Overlapping or duplicate containing rows raise `AmbiguousMunicipioError`; its
`candidates` attribute retains every queried row, including geometry and source
attributes. Equal names do not resolve that ambiguity. Nonfinite/out-of-range
coordinates and a containing row without a nonempty string name raise
`ValueError`. Boundary-only contact is excluded by `ST_Contains`. These labels
are spatial lookup results, not independent canonical identity bindings.
