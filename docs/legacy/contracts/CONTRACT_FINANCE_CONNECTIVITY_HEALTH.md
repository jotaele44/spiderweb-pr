# Contract-Finance Connectivity Health — Contract-Sweeper → spiderweb-pr

**Assessed:** 2026-05-28 · **Re-verified after fix:** 2026-05-29
**Scope:** the *financial data handoff* — the federation **export package** that
Contract-Sweeper (producer) ships and the spiderweb-pr `contract_finance` layer
(consumer) ingests.
...
```
Contract-Sweeper (producer)                         spiderweb-pr (consumer / query hub)
  scripts/build_export_package.py                     federation/hub/adapters/contract_sweeper.py
  schemas/contract_sweeper_*.schema.json   ── pkg ─►   readiness/contract_sweeper_package_gate.py
...
exercised its own fixture (the consumer's `tests/fixtures/contract_sweeper_v1_1/`
encoded a shape Contract-Sweeper never emits).
...
5. **Hazard (not on the wire).** Contract-Sweeper's
   `contract_sweeper/federation/{envelope,export_writer,validator}.py` is a separate
...
Contract-Sweeper's `scripts/validate_export.py`). Both repos bumped `1.1.0 → 1.2.0`
(a coordination bump; no field changes vs. the producer's real 1.1.0).

**Producer (Contract-Sweeper):** version bump in `scripts/build_export_package.py`,
`schemas/contract_sweeper_export_manifest.schema.json`, `exports/samples/`, docs;
legacy envelope track annotated; new non-synthetic conformance package committed at
`exports/conformance/v1_2/`.

**Consumer (spiderweb-pr):** `federation/hub/adapters/contract_sweeper.py` now reads
...
`readiness/contract_sweeper_package_gate.py` reads the canonical coordinates;
...
- A single **non-synthetic v1.2.0 conformance package**, byte-identical in both repos:
  ...
  - consumer: `tests/fixtures/contract_sweeper_v1_2/`
- `tests/test_contract_sweeper_conformance.py` (consumer): loads it in **production**
