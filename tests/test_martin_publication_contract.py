...
def test_published_source_is_admitted_to_production():
    ...
    assert prod_manifest["admitted_sources"] == ["municipios"]
...
def test_promotion_checker_is_non_mutating_and_only_reports_eligibility():
    ...
    receipt = check("municipios", "quarantined")
    after = (ROOT / "configs" / "martin_delivery.yaml").read_bytes()
    assert before == after
    assert receipt["current_state"] == "published"
    assert receipt["target_state"] == "quarantined"
    assert receipt["mutation_performed"] is False


def test_invalid_transition_rejected():
    with pytest.raises(ValueError, match="invalid transition"):
        check("municipios", "candidate")


def test_production_ingress_authorizes_municipios_when_published():
    assert published_source_ids() == {"municipios"}
