...
def test_gap_terms_preserve_uncertainty():
    # "track gap" is a real alias of the UNKNOWN blackout class — a successful
    # resolution, distinguished from a failed lookup by resolution_status.
...
def test_unresolved_lookups_use_distinct_sentinel():
    blackout = normalize_blackout("no such gap term", CONFIG_DIR)
    assert blackout["blackout_class"] == "UNRESOLVED"
    assert blackout["resolution_status"] == "unresolved"
    assert blackout["raw_text"] == "no such gap term"

    mission = normalize_mission("no such mission term", CONFIG_DIR)
    assert mission["mission_canonical"] == "UNRESOLVED"
    assert mission["resolution_status"] == "unresolved"
