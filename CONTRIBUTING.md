...
# Contributing to spiderweb-pr

Thanks for helping improve `spiderweb-pr` — the spatial and operational producer, covering FR24 OCR/GIS flight intelligence and the GEBCO bathymetry pipeline in the Puerto Rico
...
| Template drift | Run `python3 tools/render_federation_templates.py --repo spiderweb-pr --repo-root /path/to/spiderweb-pr --check` from a local Hub checkout. |
