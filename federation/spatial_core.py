...
def geodesic_distance_m(lon1: float, lat1: float, lon2: float, lat2: float) -> float:
    ...
    U1 = math.atan((1-WGS84_F)*math.tan(phi1)); U2 = math.atan((1-WGS84_F)*math.tan(phi2))
    L = math.radians(lon2-lon1); lam = L
    for _ in range(200):
        sl, cl = math.sin(lam), math.cos(lam)
        ss = math.sqrt((math.cos(U2)*sl)**2 + (math.cos(U1)*math.sin(U2)-math.sin(U1)*math.cos(U2)*cl)**2)
        if ss == 0: return 0.0
        cs = math.sin(U1)*math.sin(U2)+math.cos(U1)*math.cos(U2)*cl
        sigma = math.atan2(ss, cs); sa = math.cos(U1)*math.cos(U2)*sl/ss
        c2a = 1-sa*sa
        c2sm = 0.0 if c2a == 0 else cs - 2*math.sin(U1)*math.sin(U2)/c2a
        C = WGS84_F/16*c2a*(4+WGS84_F*(4-3*c2a))
        nxt = L + (1-C)*WGS84_F*sa*(sigma + C*ss*(c2sm + C*cs*(-1+2*c2sm*c2sm)))
...
def point_in_bbox(lon: float, lat: float, bbox: Sequence[float]) -> bool:
    ...
    if len(bbox) != 4: raise ValueError("bbox must be [min_lon,min_lat,max_lon,max_lat]")
...
def bbox_distance_m(lon: float, lat: float, bbox: Sequence[float]) -> float:
    ...
    min_lon,min_lat,max_lon,max_lat = map(float,bbox)
    near_lon = min(max(lon,min_lon),max_lon); near_lat = min(max(lat,min_lat),max_lat)
    return geodesic_distance_m(lon,lat,near_lon,near_lat)

@dataclass(frozen=True)
class TrackPoint4D:
    lon: float; lat: float; altitude_m: float | None = None; epoch_s: float | None = None
...
def segment_metrics_4d(a: TrackPoint4D, b: TrackPoint4D) -> dict[str, float | None]:
    ...
    vertical = None if a.altitude_m is None or b.altitude_m is None else b.altitude_m-a.altitude_m
    ...
    return {"horizontal_m":horizontal,"vertical_m":vertical,"distance_3d_m":distance_3d,"elapsed_s":elapsed,"speed_mps":None if not elapsed or elapsed <= 0 else distance_3d/elapsed}
