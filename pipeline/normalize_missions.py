...
# Fallback for lookups that match no alias. Deliberately NOT a vocabulary member:
# both mission_vocab and blackout_vocab define a real UNKNOWN class (e.g. "track gap"
# legitimately resolves to the UNKNOWN blackout class), so unresolved lookups must
# use a distinct sentinel and callers must key off resolution_status, not the class.
...
def normalize_mission(raw_text: str, config_dir: Path = Path("configs")) -> Dict[str, Any]:
    ...
    return {
        "raw_text": raw_text,
        "mission_canonical": canonical or UNRESOLVED,
        "resolution_status": "resolved" if canonical else "unresolved",
...
def normalize_blackout(raw_text: str, config_dir: Path = Path("configs")) -> Dict[str, Any]:
    ...
    return {
        "raw_text": raw_text,
        "blackout_class": canonical or UNRESOLVED,
        "resolution_status": "resolved" if canonical else "unresolved",
